            ...
            <div class="row layout-top-spacing">
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuLink1">
                                                ...
                                                <a class="dropdown-item view-details" data-bs-toggle="modal" data-bs-target="#editDialog" href="#" onclick='editModal(
                                                {{$shortcode->id}},
                        ...
                        <div class="modal fade" id="editDialog" tabindex="-1" role="dialog" aria-labelledby="editDialogTitle" aria-hidden="true">
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                ...
                                <p class="modal-text">In hac habitasse platea dictumst. Proin sollicitudilacus in tincidunt. Integer nisl ex, sollicitudin eget nulla nec, pharlacinia nisl. Aenean nec nunc ex. Integer varius neque at dolor sceleriporttitor.</p> -->
                                        <form method="post" id="shortcodeEditForm" action='/admin/shortcodes/update'>
                                            @csrf
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <!-- <h4 class="modal-heading mb-4 mt-2">Aligned Center</h4>
                                <p class="modal-text">In hac habitasse platea dictumst. Proin sollicitudilacus in tincidunt. Integer nisl ex, sollicitudin eget nulla nec, pharlacinia nisl. Aenean nec nunc ex. Integer varius neque at dolor sceleriporttitor.</p> -->
                                        <form method="post" id="shortcodeCreateForm" action='/admin/shortcodes/create'>
                                            @csrf
                                            <div class="form-group">
                                                <label for="exampleFormControlInput1">Shortcode Name</label>
                ...
                <x-slot:footerFiles>
                    ...
                    <script>
                        ...
                        document.getElementById('shortcodeCreateForm').addEventListener('submit', function(event) {
                            ...
                            xhr.onload = function() {
                                if (xhr.status === 200) {
                                    ...
                                    var shortcode = newRow.insertCell(1);
                                    var shortcode_category = newRow.insertCell(2);
                                    var required = newRow.insertCell(3);
                                    var enabled = newRow.insertCell(4);
                                    var show = newRow.insertCell(5);
                                    var display = newRow.insertCell(6);
                                    var position = newRow.insertCell(7);
                                    var action = newRow.insertCell(8);

                                    nameCell.innerHTML = response.name;
                                    shortcode.innerHTML = response.shortcode;
                                    shortcode_category.innerHTML = response.shortcode_category;
                                    required.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.required ? 'checked' : ''} disabled></div>`;
                                    enabled.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.enabled ? 'checked' : ''} disabled></div>`;
                                    show.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" disabled></div>`;
                                    display.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" disabled></div>`;
                                    position.innerHTML = response.position;
                                    ...
                                    document.getElementById('shortcodeCreateForm').reset();
                                    // Close the modal
                        ...
                        const editForm = document.getElementById('shortcodeEditForm');
                        ...
                        editForm.addEventListener('submit', function(event) {
                            ...
                            xhr.onload = function() {
                                if (xhr.status === 200) {
                                    // Form submitted successfully
                                    var response = JSON.parse(xhr.responseText);
                                    editForm.elements['id'].value = response.id;
                                    editForm.elements['name'].value = response.name;
                                    editForm.elements['shortcode'].value = response.shortcode;
                                    // editForm.elements['category'].value = response.shortcode_category;
                                    editForm.elements['required'].value = response.required;
                                    editForm.elements['enable'].value = response.enable;
                                    editForm.elements['show_to_dashboard'].value = response.show_to_dashboard;
                                    editForm.elements['display_on_wp'].value = response.display_on_wp;
                                    editForm.elements['full'].value = response.full;
                                    ...
                                    var shortcode = rowToUpdate.cells[1];
                                    shortcode.textContent = response.shortcode;

                                    var category = rowToUpdate.cells[2];
                                    category.textContent = response.shortcode_category;

                                    var required = rowToUpdate.cells[3];
                                    required.innerHTML =  `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.required ? 'checked' : ''} disabled></div>`;

                                    var enable = rowToUpdate.cells[4];
                                    enable.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.enable ? 'checked' : ''} disabled></div>`;

                                    var show_to_dashboard = rowToUpdate.cells[5];
                                    show_to_dashboard.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.show_to_dashboard ? 'checked' : ''} disabled></div>`;

                                    var display_on_wp = rowToUpdate.cells[6];
                                    display_on_wp.innerHTML = `<div class="form-check form-check-primary form-check-inline"><input class="form-check-input" type="checkbox" name="required" ${response.display_on_wp ? 'checked' : ''} disabled></div>`;

                                    var position = rowToUpdate.cells[7];
                                    position.textContent = response.position;
